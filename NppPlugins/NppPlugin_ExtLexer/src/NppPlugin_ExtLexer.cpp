...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD reasonForCall, LPVOID /*lpReserved*/)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		pIface::init(TEXT("NppExternalLexers"), hModule);
		...
		pIface::setPluginFuncItem(TEXT("About..."), plugin::About_func);
		...
		lIface::init( "Conf*", TEXT("Apache Config File. *Ext"),
			l_conf::LexOrFold, l_conf::menuDlg );

		lIface::init( "MYUSERLANG*", TEXT("MYUSERLANG File. *Ext"),
			l_myuserlang::LexOrFold, l_myuserlang::menuDlg );

		lIface::init( "Template*", TEXT("Lexer Template File. *Ext"), 
			l_template::LexOrFold, l_template::menuDlg);

		lIface::init( "PowerShell*", TEXT("PowerShell Scipt File. *Ext"), 
...
void NppExtLexer_Plugin::Help_func()
{
	...
	bool docOpened = ( 1 == SendMessage( pIface::hNpp(), NPPM_DOOPEN, 0, (LPARAM)pluginHelpFile.c_str() ) );
