...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD reasonForCall, LPVOID /*lpReserved*/)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		npp_plugin::initPlugin(TEXT("Plugin Template"), hModule);
		...
		//  Test retrieval of xml config data
		{
			tstring confVal = npp_plugin::xmlconfig::getGUIConfigValue(TEXT("TestValue"), TEXT("readValue"));
			if ( confVal.compare(TEXT("readme")) == 0 )
			{
				bool tstVal = true;
			}
			else
		...
		//  Test writing of xml config data
		{
			tstring confVal = TEXT("writeme");
			npp_plugin::xmlconfig::setGUIConfigValue(TEXT("TestValue"), TEXT("writeValue"), confVal);
			if ( npp_plugin::xmlconfig::getGUIConfigValue(TEXT("TestValue"), TEXT("writeValue")) == TEXT("writeme")) {
				bool tstVal = true;
			}
			else {
				bool tstVal = false;
...
void npp_plugin::About_func() 
{
	::MessageBox(npp_plugin::hNpp(),
		TEXT("  This is just a test template project file.")
